...
Every checkout is validated with `/usr/bin/git` before display and again before an editor is opened. Before launching Zed, the extension invokes an explicitly allowlisted interactive login shell with a fixed command, extracts only its `PATH`, and passes that value to the Zed CLI. Unsupported login shells fall back to `/bin/zsh`. All other external commands use argument arrays with shell execution disabled.
