...
每个 checkout 在展示前会通过 `/usr/bin/git` 验证，在打开编辑器前会再次验证。启动 Zed 前，扩展会使用固定命令调用显式白名单中的交互式登录 Shell，仅提取其中的 `PATH` 并传给 Zed CLI；不支持的登录 Shell 会回退到 `/bin/zsh`。其余外部命令均通过参数数组调用，并禁用 shell 执行。
